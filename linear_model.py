...
#x_train.to_csv("data\\x_train_l.csv", sep=',', encoding='utf-8', header=False, na_rep='', index=False)
#x_test.to_csv("data\\x_test_l.csv", sep=',', encoding='utf-8', header=False, na_rep='', index=False)
...
linear_columns = [dept_buckets, endsite_buckets, m_buckets, w_buckets]

model = tf.estimator.LinearRegressor(
		model_dir = 'tmp\\model_Linear',
...
model.train(input_fn=lambda: input_fn("data\\x_train_l.csv", 40, True, 10))

result = model.evaluate(input_fn=lambda: input_fn("data\\x_test_l.csv", 1, False, 10))
for key in sorted(result):
		print('%s: %s' % (key, result[key]))
	

predictResult = model.predict(input_fn=lambda: input_fn("data\\x_test_l.csv", 1, False, 10))
